#!/usr/bin/env python3
"""Retired compatibility builder for the pre-1.8.3 flight animation layout.

The current stack is generated by ``BuildFlightAnimationStack.py``.  Keeping
this legacy entry point importable is useful for old automation, but running
it against a checkout would create obsolete family names/scopes and could
poison the current OAR coverage and hash manifests, so ``main`` refuses to
mutate any tree.
"""
    ...
    for index, argument in enumerate(args[:-1]):
        if argument in {"-o", "--output"}:
            ...
            if not output_path.is_file() or output_path.stat().st_size == 0:
                raise RuntimeError(
                    f"hkxc completed without producing its declared output: {output_path}"
                )
            break
...
def write_hash_manifest(data_root: pathlib.Path) -> int:
    ...
    lines = [
        "Dragon Aspect Flight 1.8.3 bundled animation/effect asset SHA-256",
        "=================================================================",
        "",
    ]
    lines.extend(
...
def main() -> int:
    ...
    raise RuntimeError(
        "BuildFlightBaseAnimations.py is retired and intentionally refuses to "
        "write the current stack; use BuildFlightAnimationStack.py instead."
    )
    if not hkxc.is_file():
