...
- Open Animation Replacer selects external More Draconic flight animations only when `bDAF_FlightActive == true` and `iDAF_FlightState > 0`.
...
The Dragon Aspect Flight package owns selector configs only. Those configs use OAR's `overrideAnimationsFolder` to load clips from the sibling More Draconic OAR package at `More Dragonic Dragon Aspect Can Fly\Flying Mod`, so the More Draconic animation mod remains the owner of the HKX files.
